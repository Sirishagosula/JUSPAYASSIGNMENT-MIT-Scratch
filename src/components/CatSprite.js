...
export default function CatSprite({ inputValues,repeatCount }) {
  ...
  useEffect(() => {
    ...
    async function runAnimations() {
      ...
      for (let i = 0; i < inputValues?.length; i++) {
        ...
        else if (key.key === "hide" && key.value === 0) {
          toggleHide();
        } else if (key.key === "hide" && key.value === 1) {
          toggleHide();
        }
        ...
        else if (key.key.startsWith("SayMessage")) {
        ...
        else if (key.key.startsWith("hide")) {
         toggleHide();
        }
        else if (key.key.startsWith("BroadcastMessage")) {
          const message = key.value;
          setMessage(message);
          setShowMessage(true);
    ...
    runAnimations();
  }, [inputValues,repeatCount,isHidden]);
